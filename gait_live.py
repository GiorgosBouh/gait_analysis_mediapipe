...
def run_live_gait_analysis():
    ...
    if "recorded_data" in st.session_state and not st.session_state.recorded_data:
        st.session_state.recorded_data = {
            "left_xs": [], "left_ys": [],
            "right_xs": [], "right_ys": [],
            "joints": {
    ...
    if st.session_state.recording:
        st.success("🔴 Recording is in progress...")
    ...
    if not st.session_state.recording and st.session_state.recorded_data.get("frame_idx", 0) > 0:
        rd = st.session_state.recorded_data
        st.write(f"📄 CSV: `{rd['csv_path']}`")
        st.write(f"🎥 Video: `{rd['video_path']}`")
        st.success("✅ Gait features would go here.")