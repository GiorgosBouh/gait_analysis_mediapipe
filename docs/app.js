import { createPoseLandmarker, POSE_LANDMARK_NAMES, POSE_CONNECTIONS, LANDMARK_INDEX } from "./pose.js";
import { clamp, median, movingAverage, downloadFile, formatNumber, computeDtStats, ensureCanvasSize, computePeaks } from "./utils.js";
...
function updateResults() {
  const steps = appState.data.map((frame) => frame.step_length_m2d_apparent).filter((v) => Number.isFinite(v));
...
function computeCadence() {
  const trajectory = appState.data
    .map((frame) => frame.left_ankle_x_m2d)
...
function setVisibilityStatus(isFullBody) {
  ...
  if (!isFullBody && !appState.warnings.includes("Body not fully visible")) {
    appState.warnings.push("Body not fully visible");
  }
}
...
function updateCalibration(landmarks) {
  ...
  if (pixelHeight) {
    appState.calibration.samples.push(pixelHeight);
  }
  ...
  if (appState.calibration.totalFrames >= appState.calibration.targetFrames) {
    ...
    if (validCount >= required && med && med > 0.25 * appState.videoSize.height) {
      ...
      if (!appState.warnings.includes("Calibration failed; 2D meters disabled")) {
        appState.warnings.push("Calibration failed; 2D meters disabled");
      }
    }
  }
}
...
function getMetricLandmarks(pixelLandmarks, scale) {
  if (!scale) {
    return pixelLandmarks.map(() => ({ x: NaN, y: NaN }));
  }
...
function exportCsv() {
  ...
  POSE_LANDMARK_NAMES.forEach((name) => {
    header.push(`${name}_x_px`, `${name}_y_px`);
  });
  POSE_LANDMARK_NAMES.forEach((name) => {
    header.push(`${name}_x_m2d`, `${name}_y_m2d`);
  });
  POSE_LANDMARK_NAMES.forEach((name) => {
    header.push(`${name}_wx`, `${name}_wy`, `${name}_wz`);
  });
  header.push(
    "step_width_m2d",
  ...
  const rows = appState.data.map((frame) => {
    const row = [frame.timestamp_s.toFixed(4)];
...
function runLoop() {
  ...
  const throttle = Number(dom.inferenceThrottle.value);

  if (appState.data.length % throttle === 0) {
...
function validateHeight() {
  ...
  if (appState.calibration.scale) {
    ...
    if (latestMedian) {
      appState.calibration.scale = heightMeters / latestMedian;
    }
  }
  updateStatusUI();
}
...
function setupEventListeners() {
  ...
  dom.smoothingWindow.addEventListener("input", updateSmoothingLabel);
